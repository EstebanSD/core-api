...
import { KeywordsStreamUseCase } from './keywords-stream.use-case';

describe('KeywordsStreamUseCase', () => {
  let useCase: KeywordsStreamUseCase;

  let mockStreamText: jest.MockedFunction<NonNullable<AIProvider['streamText']>>;
  let mockPromptBuilder: jest.Mocked<KeywordsPromptBuilder>;
  ...
  beforeEach(() => {
    ...
    mockPromptBuilder = {
      build: mockBuild,
    } as unknown as jest.Mocked<KeywordsPromptBuilder>;

    useCase = new KeywordsStreamUseCase(provider, mockPromptBuilder);
  });
  ...
  it('should build prompt and call provider stream', async () => {
    ...
    const stream = useCase.execute(content, limit);
    ...
    expect(mockStreamText).toHaveBeenCalledWith({
      ...
      maxTokens: 150,
      temperature: 0.2,
      metadata: { operation: 'keywords-stream' },
    });
  ...
  it('should propagate provider errors', async () => {
    ...
    const stream = useCase.execute('content', 5);
  ...
  it('should throw if provider does not support streaming', () => {
    ...
    const useCase = new KeywordsStreamUseCase(provider, mockPromptBuilder);

    expect(() => useCase.execute('content', 5)).toThrow(AIUseCaseError);
  });
});
