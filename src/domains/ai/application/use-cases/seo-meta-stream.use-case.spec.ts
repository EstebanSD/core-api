...
import { SeoMetaStreamUseCase } from './seo-meta-stream.use-case';

describe('SeoMetaStreamUseCase', () => {
  let useCase: SeoMetaStreamUseCase;

  let mockStreamText: jest.MockedFunction<NonNullable<AIProvider['streamText']>>;
  let mockPromptBuilder: jest.Mocked<SeoMetaPromptBuilder>;
  let mockBuild: jest.Mock;

  beforeEach(() => {
    const { provider, mockStreamText: mock } = createMockAIStreamProvider([
      { delta: 'Mocked ', done: false },
      { delta: 'SEO Meta', done: false },
      { delta: '', done: true },
    ]);
    ...
    mockPromptBuilder = {
      build: mockBuild,
    } as unknown as jest.Mocked<SeoMetaPromptBuilder>;

    useCase = new SeoMetaStreamUseCase(provider, mockPromptBuilder);
  });
  ...
  it('should build prompt and call provider stream', async () => {
    ...
    expect(mockStreamText).toHaveBeenCalledWith({
      ...
      maxTokens: 200,
      temperature: 0.6,
      metadata: { operation: 'seo-meta-stream' },
    });
  ...
  it('should throw if provider does not support streaming', () => {
    ...
    const useCase = new SeoMetaStreamUseCase(provider, mockPromptBuilder);
